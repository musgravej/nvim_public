...
return {
    ...
    config = function()
        require("persisted").setup({
            ...
            allowed_dirs = {"~/github", "~/.config"}, -- Table of dirs that the plugin will start and autoload from
            ignored_dirs = {"~/Downloads"}, -- Table of dirs that are ignored for starting and autoloading
            telescope = {
        ...
        vim.keymap.set("n", "<leader>pt", "<cmd>Persisted toggle<cr>", { desc = "[P]ersisted [t]oggle" })
        vim.keymap.set("n", "<leader>pa", "<cmd>Persisted start<cr>", { desc = "[P]ersisted st[a]rt" })
        vim.keymap.set("n", "<leader>po", "<cmd>Persisted stop<cr>", { desc = "[P]ersisted st[o]p" })
        vim.keymap.set("n", "<leader>ps", "<cmd>Persisted save<cr>", { desc = "[P]ersisted [s]ave" })
        vim.keymap.set("n", "<leader>pl", "<cmd>Persisted select<cr>", { desc = "[P]ersisted [l]ist" })
        vim.keymap.set("n", "<leader>pm", "<cmd>Telescope persisted<cr>", { desc = "[P]ersisted [m]anage" })
    end,
}
